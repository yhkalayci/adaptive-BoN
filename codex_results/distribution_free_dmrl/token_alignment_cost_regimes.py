"""Replay token-count alignment at chosen prices and make four tables/figures.
...
from replay_token_reward_farm import (
    ...
    load_model, prefix_thresholds, sigmoid,
)
...
PRICES_PER_MILLION = np.array([0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0])


def replay(rewards, lengths):
    """One set of seeded response orders serves all requested prices."""
    prices = PRICES_PER_MILLION / 1e6
    adaptive = np.zeros((len(rewards), len(prices), 3))
    fixed = np.zeros((len(rewards), CAP, 3))
    ...
    for i in range(len(rewards)):
        ...
        for _ in range(PERMUTATIONS):
            ...
            thresholds = prefix_thresholds(r, length)
            stopped = np.array([np.flatnonzero(thresholds <= p)[0]
                                if np.any(thresholds <= p) else CAP - 1 for p in prices])
...
def summarize(model, reward_key, adaptive, fixed):
    ...
    for seed in SPLIT_SEEDS:
        ...
        for pi, price_per_million in enumerate(PRICES_PER_MILLION):
            ...
            fixed_profit = test_fixed[best_n, 0] - price * test_fixed[best_n, 1]
            ...
            rows.append(dict(model=model, reward_model=reward_key, split_seed=seed,
                             ...
                             profit=float(profit), fixed_profit=float(fixed_profit),
                             profit_gain=float(profit - fixed_profit),
                             profit_improvement_percent=(
                                 float(100 * (profit - fixed_profit) / fixed_profit)
                                 if fixed_profit > 0 else None),
    ...
    for reward in REWARDS:
        for model in MODELS:
            ...
            for si, (train, test) in enumerate(split_indices):
                ...
                split_point[si] = (a_profit - f_profit).mean(axis=0)
                w = weights[:, test].copy()
                w /= w.sum(axis=1, keepdims=True)
                boot += w @ (a_profit - f_profit)
    ...
    for reward in REWARDS:
        for pi, price_per_million in enumerate(PRICES_PER_MILLION):
            ...
            for key in ("fixed_n", "adaptive_utility", "fixed_utility",
                        ...
                        "adaptive_profit", "fixed_profit", "profit_gain",
                        "direct_cost_saving_percent", "matched_quality_cost_saving_percent"):
                table_row[key] = float(np.mean([r[key] for r in chosen]))
            percents = [r["profit_improvement_percent"] for r in chosen]
            table_row["profit_improvement_percent"] = (
                float(np.mean(percents)) if all(x is not None for x in percents) else None)
...
def draw_figures(intervals, output):
    ...
    for reward in REWARDS:
        ...
        for mi, model in enumerate(MODELS):
            ...
            y = np.array([r["profit_gain"] for r in group])
        ...
        ax.set_ylabel("Adaptive − fixed-N profit ($ per prompt)")
...
def write_markdown(tables, output):
    lines = ["# Token-count adaptive alignment replay", "",
             ...
             "Profit gain is adaptive minus fixed-N profit in dollars per prompt; this absolute "
             "difference stays meaningful when fixed-N profit is zero or negative.", "",
    ...
    for reward in REWARDS:
        ...
        for r in tables[reward]:
            lines.append("| " + " | ".join((
                ...
                f"{r['adaptive_profit']:.4f}", f"{r['profit_gain']:+.5f}",
    ...
    lines += ["Figure bands are pointwise 95% paired prompt-bootstrap percentile intervals "
              "for each generator's absolute profit gain against its own train-selected fixed N. "
              ...
              "Figures show all four generators, whereas tables use training-selected generators. "
              "Relative profit percentages are blank in the CSV wherever fixed-N profit is "
              "nonpositive.", ""]
    (output / "REPORT.md").write_text("\n".join(lines))


def main():
    global PRICES_PER_MILLION
    ...
    parser.add_argument("--prices-per-million", nargs="+", type=float,
                        default=PRICES_PER_MILLION.tolist(),
                        help="strictly increasing dollar prices per million output tokens")
    ...
    PRICES_PER_MILLION = np.asarray(args.prices_per_million, dtype=float)
    if (not np.all(np.isfinite(PRICES_PER_MILLION)) or
            np.any(PRICES_PER_MILLION <= 0) or
            np.any(np.diff(PRICES_PER_MILLION) <= 0)):
        parser.error("--prices-per-million must be finite, positive, and strictly increasing")
    ...
    method = dict(input_root=str(args.input_root.resolve()),
                  ...
                  profit_gain="adaptive profit minus train-selected fixed-N profit, in dollars per prompt",
                  intervals="pointwise paired prompt-bootstrap of absolute profit differences; shared weights across splits and generators; conditional on training selections",
